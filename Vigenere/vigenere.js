...
if (mode.startsWith('en')) {
  ...
  const key = vigenereCrack(text, alpha, nativeText);
...
function vigenereCrack(text, alpha, nativeText) {
  let filtred = filterText(text, alpha);

  let keyLength = computeKeyLength(filtred, alpha);

  let key = bruteKey(filtred, alpha, keyLength, nativeText);

  return key;
}

function bruteKey(filteredText, alpha, keyLength, nativeText) {
  let tetraFrequencies = calculateTetra(nativeText, alpha);

  let words = fs.readFileSync('words', 'utf8')
    .split('\n')
  ...
  function calculateTetra(nativeText, alpha) {
    ...
    const SAMPLE_SIZE = Math.min(nativeText.length, 300);

    for (let i = 0; i < convert(alpha.card, 1, 0, 0, 0, 0); ++i)
      freq.push(0);
